...
old_str = '''              if (msg.text.startsWith("__system_audio:")) {
                const audioBase64 = msg.text.replace("__system_audio:", "");
                return (
                  <div key={idx} className={lex flex-col max-w-[75%] mb-1 }>
                    <div onClick={() => setClickedMessageIdx(clickedMessageIdx === idx ? null : idx)} className={py-2 px-3 rounded-xl shadow-sm relative group cursor-pointer transition-all }>
...
if old_str in content_norm:
    content_norm = content_norm.replace(old_str, new_str)
