...
const ChatWrapper: React.FC<{ onLogout: () => void; token: string }> = ({ onLogout, token }) => {
  ...
  const { sendMessage: sendWsMessage, sendRawMessage, socketStatus } = useWebSocket(
    ...
    (parsed: any) => {
      try {
        if (["call_offer", "call_answer", "ice_candidate", "call_reject", "call_end", "call_ring", "call_ring_offline", "call_ring_remote", "call_accepting", "call_connected"].includes(parsed.type)) {
